...
**Verification status:** source reviewed. The `infra-validate` GitHub workflow passed `terraform fmt -check`, `terraform init -backend=false` and `terraform validate` on 2026-09-17 (Terraform 1.16.3, hashicorp/aws 6.65.0). No lock file is committed, no plan was produced and nothing was applied. Do not interpret the architecture diagram as implemented IaC coverage.
